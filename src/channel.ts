...
import {
  ...
  resolveDefaultBitrix24AccountId,
} from "./accounts.js";
import { Bitrix24Client } from "./client.js";

const meta = getChatChannelMeta("bitrix24");

export const bitrix24Plugin: ChannelPlugin<ResolvedBitrix24Account> = {
  id: "bitrix24",
  meta: {
    ...meta,
  ...
  config: {
    ...
    setAccountEnabled: ({ cfg, accountId, enabled }) => {
      ...
      if (!next.channels.bitrix24) next.channels.bitrix24 = {};
      
      if (accountId === "default") {
        next.channels.bitrix24.enabled = enabled;
      } else {
        if (!next.channels.bitrix24.accounts) next.channels.bitrix24.accounts = {};
    ...
    deleteAccount: ({ cfg, accountId }) => {
      const next = { ...cfg };
      if (!next.channels?.bitrix24) return next;
 ...
 */
async function getClientForAccount(accountId?: string): Promise<Bitrix24Client> {
  const cfg = await getBitrix24Runtime().config.loadConfig();
  ...
  return new Bitrix24Client({
    ...
    userId: account.userId,
    botId: account.botId,
    clientId: account.clientId, // For bot API calls
    log: getBitrix24Runtime().logging,
  });
}
